    ...
    foreach (mp_color_catalog_product_files() as $path) {
        ...
        foreach ($product['steps'] as $step) {
            ...
            $flows[$key] = array(
                ...
                'stepId' => $stepId,
                'label' => trim((string)(isset($product['title']) ? $product['title'] : ucfirst($productSlug)))
                    . ' — ' . trim((string)(isset($step['title']) ? $step['title'] : $stepId)),
                'colors' => $flowColors,
            );
        }
    }
...
function mp_color_catalog_seed()
{
    ...
    $now = mp_db_now();
    $insertColor = $pdo->prepare('INSERT OR IGNORE INTO colors (id, name, hex, sort_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)');
    ...
    try {
        foreach ($flows as $flow) {
            foreach ($flow['colors'] as $color) {
                ...
                $insertColor->execute(array($id, $name !== '' ? $name : $id, $hex, $order++, $now, $now));
...
function mp_color_catalog_data()
{
    ...
    $pdo = mp_db();
    $flows = mp_color_catalog_flows();
    $rows = $pdo->query('SELECT id, name, hex, sort_order FROM colors ORDER BY sort_order ASC, name COLLATE NOCASE ASC')->fetchAll();
...
function mp_color_catalog_save($payload)
{
    ...
    $pdo = mp_db();
    $now = mp_db_now();
    $upsert = $pdo->prepare('INSERT INTO colors (id, name, hex, sort_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET name=excluded.name, hex=excluded.hex, sort_order=excluded.sort_order, updated_at=excluded.updated_at');
    ...
    try {
        ...
        foreach ($clean as $color) {
            $upsert->execute(array($color['id'], $color['name'], $color['hex'], $color['sort_order'], $now, $now));
