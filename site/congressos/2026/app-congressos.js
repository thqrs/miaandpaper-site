(function () {
  ...
  var CONTENT_BASE = document.body.dataset.contentBase || "";
  // As paginas de congressos vivem em /congressos/2026/, mas os <base href> do HTML
  // nao se aplicam a URLs dentro de CSS (style inline). Estes resolvem contra o URL
  // do documento, por isso os url(...) precisam do prefixo explicito.
  var ASSET_BASE = document.body.dataset.assetBase || "";
  ...
  function assetPath(value) {
    ...
    if (!ASSET_BASE || !path || /^(?:data:|https?:|\/\/|\/)/.test(path)) {
      return path;
    }
  ...
  // O catálogo público é estático e partilhado com o site principal.
  ...
  function installFavicon() {
    ...
    link.href = assetPath("content/brand/logo.webp");
    if (!link.parentNode) {
  ...
  function applySiteSettings(home) {
    ...
    if (theme.backgroundImage) {
      root.style.setProperty("--site-bg-image", 'url("' + assetPath(theme.backgroundImage).replace(/"/g, "%22") + '")');
  ...
  function enrichHomeWithCarousels(home) {
    ...
    return Promise.all((home.categories || []).map(function (category) {
      ...
      return loadJson(CONTENT_BASE + "content/products/" + slug + ".json").then(function (product) {
  ...
  function funnelEventEnabled(eventName) {
    var maximumOnly = { ui_interaction:true, dead_tap:true, image_magnified:true, heartbeat:true, selection_updated:true, catalog_scroll_depth:true, offer_scroll_depth:true, offer_image_zoom_clicked:true };
    var minimumEvents = { site_landed:true, wizard_started:true, step_view:true, step_completed:true, confirmation_view:true, validation_error:true, contact_started:true, contact_completed:true, cart_item_added:true, cart_item_updated:true, cart_checkout_started:true, cart_order_submitted:true, order_submitted:true, artwork_upload_started:true, artwork_upload_completed:true, artwork_upload_failed:true, catalog_session_started:true, catalog_page_view:true, offer_page_view:true, offer_pdf_download_clicked:true };
  ...
  function funnelSendTrackingBatch(events) {
    ...
    if (window.fetch) {
      window.fetch(FUNNEL_ENDPOINT, { method:"POST", headers:{ "Content-Type":"application/json" }, body:body, keepalive:true, credentials:"same-origin" }).catch(function () {});
    }
  }
  ...
  function funnelFlushTracking() {
    ...
    funnelTrackingQueue = funnelTrackingQueue.filter(function (payload) { return funnelEventEnabled(payload && payload.event_name); });
    ...
    if (funnelTrackingQueue.length) funnelTrackingFlushTimer = window.setTimeout(funnelFlushTracking, 0);
  }
  ...
  function funnelQueueTracking(payload) {
    ...
    if (!funnelTrackingReady) return;
    if (funnelTrackingQueue.length >= funnelTrackingBatchMaxEvents) { funnelFlushTracking(); return; }
    window.clearTimeout(funnelTrackingFlushTimer);
  ...
  function funnelLoadTrackingConfig() {
    ...
    }).catch(function () { funnelTrackingLevel = "medium"; }).then(function () {
  ...
  document.addEventListener("visibilitychange", function () { if (document.hidden) funnelFlushTracking(); });
    ...
    return [
      ...
      images.map(function (image, index) {
        ...
        return '<span class="category-carousel-frame' + (index === 0 ? ' is-active' : '')
          ...
          + '" style="background-image:url(&quot;' + escapeHtml(assetPath(image)) + '&quot;);--carousel-pan-x:' + panX + '%;--carousel-pan-y:' + panY + '%"></span>';
      }).join(""),
      '</span>'
    ].join("");
  }
  ...
  function renderHome(home) {
    ...
    function renderHomeCategoryCard(record, displayIndex) {
      ...
      var imageStyle = hasStaticImage ? ' style="--category-image:url(&quot;' + escapeHtml(assetPath(category.image)) + '&quot;)"' : carouselStyle;
    ...
    if (home.layout !== "brand-home") {
      ...
      heroStyle = heroImage
        ? ' style="--home-hero-image:url(&quot;' + escapeHtml(assetPath(heroImage)) + '&quot;);--home-hero-position:' + escapeHtml(heroPosition) + '"'
        : "";
  ...
  function uploadedFrameInfo(item, template, step) {
    ...
    return {
      style: [
        '--uploaded-image:url(&quot;' + escapeHtml(assetPath(item.image)) + '&quot;)',
  ...
  function uploadedSideFrameStyle(item) {
    ...
    return [
      '--uploaded-image:url(&quot;' + escapeHtml(assetPath(item.sideImage)) + '&quot;)',
      "--image-zoom-scale:" + imageZoom,
      "--image-position-x:" + imagePositionX + "%",
      "--image-position-y:" + imagePositionY + "%",
      "--image-rotation:" + imageRotation + "deg",
    ...
    return [
      ...
      frames.map(function (frame, index) {
        return '<span class="cadernos-cover-preview-slide' + (index === 0 ? ' is-active' : '')
          ...
          + '" style="background-image:url(&quot;' + escapeHtml(assetPath(frame.image)) + '&quot;)"></span>';
      }).join(""),
  ...
  function renderInteriorSlideshow(product) {
    ...
    return [
      ...
      images.map(function (image, index) {
        return '<span class="interior-slide" style="background-image:url(&quot;' + escapeHtml(assetPath(image)) + '&quot;);--interior-slide-index:' + index + '"></span>';
      }).join(""),
      '</div>',
  ...
  function initHome() {
    Promise.all([
      ...
      homeContentPath === CONTENT_BASE + "content/home.json" ? Promise.resolve(null) : loadJson(CONTENT_BASE + "content/home.json").catch(function () { return null; })
  ...
  function initProduct() {
    ...
    Promise.all([
      loadJson(CONTENT_BASE + "content/products/" + productSlug + ".json"),
      loadJson(CONTENT_BASE + "content/pricing.json").catch(function () { return null; }),
      loadJson(ORDER_HOME_CONTENT).catch(function () { return null; }),
      loadJson(CONTENT_BASE + "content/home.json").catch(function () { return null; }),
  ...
  if (page === "home") {
    ...
    refreshCartUi();
    applyTheme(currentTheme());
    loadJson(CONTENT_BASE + "content/home.json").then(function (home) {
      applySiteSettings(home);
